	 ...
	 uint16_t alpha)
{
	...
	if (WARN(bpc > 16, "Cannot support RGBA values > 16bpc"))
		bpc = 16;
...
drm_rgba_bits(struct drm_rgba c, unsigned compshift, unsigned bits) {
	...
	if (WARN(bits > 16, "Cannot support RGBA values > 16bpc"))
		bits = 16;
 ...
 */
#define DRM_RGBA_REDBITS(c, bits)   drm_rgba_bits(c, 0, bits)
#define DRM_RGBA_GREENBITS(c, bits) drm_rgba_bits(c, 16, bits)
